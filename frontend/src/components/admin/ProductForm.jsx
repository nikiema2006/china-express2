...
export default function ProductForm({ fields, data, onSubmit, onCancel, loading }) {
  ...
  const analyzeWithAI = async () => {
    ...
    setAnalyzing(true);
    try {
      const productData = await kimiService.analyzeProductImages(imageBase64s);
  ...
  const hasImagesWithBase64 = images.filter((img) => img.base64).length;
