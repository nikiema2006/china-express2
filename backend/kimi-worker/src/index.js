const SUPABASE_URL_BASE = 'https://bmbeahjvdiglnxfpbzyu.supabase.co';
...
export default {
  async fetch(request, env) {
    ...
    // POST /task - create a new import task
    ...
    // Cron trigger - process one queued task
          ...
          messages.push({
            ...
            content: [
              { type: 'image_url', image_url: { url: task.image_base64 } },
              {
                type: 'text',
                text: task.url
                  ? `Lien: ${task.url}\nAnalyse cette image et cherche les infos.`
                  : `Analyse cette image de produit sur les plateformes chinoises.`,
              },
            ],
          });
        } else {
